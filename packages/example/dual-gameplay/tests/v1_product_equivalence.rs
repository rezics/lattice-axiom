...
use latticeaxiom_compose::{
    LockRequest, PRODUCT_LOCK_FILE_NAME, RealizationKind, RealizedDataRootV1, default_toolchain,
    lock_workspace,
};
...
fn workspace_root() -> PathBuf {
    ...
    for _ in 0..3 {
        assert!(
            root.pop(),
            "package manifest must be nested below the workspace"
        );
    }
    root
}
...
fn reopen_final_lock() -> ReopenedFinalLockV1 {
    static REOPENED: OnceLock<ReopenedFinalLockV1> = OnceLock::new();
    REOPENED.get_or_init(compose_final_lock_fixture).clone()
}

fn compose_final_lock_fixture() -> ReopenedFinalLockV1 {
    let workspace = workspace_root();
    let directory = TestDirectory::create();
    let lock_path = directory.path().join(PRODUCT_LOCK_FILE_NAME);
    let catalog_root = directory.path().join("catalog");
    let target = preferred_host_target().expect("the test host target must be supported");
    let request = LockRequest {
        workspace_root: workspace.clone(),
        bootstrap_path: workspace.join("latticeaxiom.toml"),
    ...
    let (lock, _) = lock_workspace(&request)
        .unwrap_or_else(|error| panic!("typed final-lock fixture must compose: {error}"));
    let host = HostBuildReceipts::sealed_by_lock(&lock)
        .expect("composed lock must seal host toolchain receipts");
    ...
    let reopened = ReopenedFinalLockV1::reopen_frozen_from_cas(&lock_path, &store, &host)
        .expect("typed final-lock fixture must reopen frozen against CAS");
    let realization = lock
        .realizations
        .get(&target)
        .expect("composed lock must contain its requested target");
    for (package, package_realization) in &realization.packages {
        if package_realization.kind != RealizationKind::Data {
            continue;
        }
        let bytes = reopened
            .verified_artifact(package_realization.artifact_digest)
            .unwrap_or_else(|| panic!("verified data artifact must exist for {package}"));
        let root = RealizedDataRootV1::from_canonical_bytes_for_package(bytes, package)
            .unwrap_or_else(|error| panic!("verified data artifact for {package}: {error}"));
        assert!(
            root.files().len() > 0,
            "verified data artifact for {package} must contain frozen files"
        );
    }
    reopened
}
