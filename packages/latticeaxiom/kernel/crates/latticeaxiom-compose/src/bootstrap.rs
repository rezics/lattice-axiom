...
pub struct PackageSourceManifestV1 {
    ...
    /// Package-local Rust implementation manifests; absent for data-only sources.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub rust: Option<PackageRustSourcesV1>,
    /// Explicit source files and directories included in a later snapshot.
    pub source_inclusion: SourceInclusionPolicyV1,
}

/// Explicit implementation ownership inside a Lattice package source root.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct PackageRustSourcesV1 {
    /// Primary library manifest used by source realization.
...
impl PackageRustSourcesV1 {
    fn is_valid(&self) -> bool {
        self.members.contains(&self.entry)
            && self.members.iter().all(|path| {
                let components = path.as_str().split('/').collect::<Vec<_>>();
                components.len() == 3 && components[0] == "crates" && components[2] == "Cargo.toml"
            })
    }
}

#[cfg(test)]
mod rust_source_tests {
    use super::PackageRustSourcesV1;

    #[test]
    fn explicit_internal_entry_and_multiple_members_are_valid() {
    ...
    #[test]
    fn root_manifest_and_undeclared_entry_are_invalid() {
        for text in [
            "entry = 'Cargo.toml'\nmembers = ['Cargo.toml']",
            "entry = 'crates/runtime/Cargo.toml'\nmembers = ['crates/model/Cargo.toml']",
        ] {
            let sources: PackageRustSourcesV1 =
                toml::from_str(text).expect("syntactically valid source declaration");
            assert!(!sources.is_valid());
        }
    }
}
...
pub enum BootstrapManifestError {
    /// Rust source ownership must enumerate internal crates and include its entry.
    #[error(
        "package {package} Rust sources must include the entry and use crates/<name>/Cargo.toml"
    )]
    InvalidRustSources {
        /// Package with an invalid implementation declaration.
        package: PackageName,
    },
