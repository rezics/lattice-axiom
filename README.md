...
目前尚無程式實作。專案預設完整採用 Bevy 與成熟 upstream；只有真實可玩原型證明其無法滿足不可妥協需求時，才允許最小自研替代。Nickel 套件內核、統一 package graph 與版本化原生 ABI 是產品組合語義，從第一個垂直切片即開始；它們不取代 Bevy 的 ECS、排程器或 renderer。已接受的 ADR 是實作基線，其餘 proposed／exploration 文件會依原型與量測修正。
