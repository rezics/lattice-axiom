...
unsafe fn lose_active_focus(window: HWND, userdata: &WindowData) {
    ...
    // `ClipCursor` is process-external shared state. Release it before the
    // focus event is handed to the application: once this window is no longer
    // focused, `CursorFlags::refresh_os_cursor` deliberately skips clip
    // updates and a later `CursorGrabMode::None` request cannot free a cursor
    // that is still confined to this window.
    if let Err(error) = util::set_cursor_clip(None) {
        tracing::warn!("Failed to release cursor clip while losing focus: {error}");
    }
