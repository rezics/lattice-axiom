...
document_id: delivery.roadmap.game-engine
...
这条路线把 package control plane、dual realization 与 Bevy runtime 合成一个可验证依赖链。它既不是「先造完整 package manager」，也不是「先让官方 Bevy Plugin 跑起来再补 ABI」。每个基础阶段都尽快进入[第一个可玩 demo](first-demo.md)。
...
R5与[first demo D0–D6](first-demo.md)只证明package-driven voxel vertical slice。
