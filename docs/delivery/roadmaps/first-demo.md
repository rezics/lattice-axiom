...
document_id: delivery.roadmap.first-demo
...
D3／D4的精确内容与后续40／72方块范围见[Terrenia 方块内容规划](../../packages/terrenia/blocks/catalog.md)。
...
- `@terrenia/blocks`由D4累计18个扩展到自然世界累计40个方块，精确集合以
  [Terrenia 方块内容规划](../../packages/terrenia/blocks/catalog.md)为准；
...
完成 demo 不自动等于 ABI 1.0；冻结仍需[执行期路线图](game-engine.md)的完整 gates。
