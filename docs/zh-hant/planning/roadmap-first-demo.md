...
- Nickel 組合並輸出正規化 JSON；
- 同一 `ResolvedModuleGraph` 支援靜態建置與資料夾動態載入；
...
| 步驟 | 產物 | 出場驗收 | 估算 | 狀態 |
| --- | --- | --- | --- | --- |
| 0 | 決策、架構、來源索引與本路線圖轉正 | 互鏈一致；舊對話與草稿完成吸收 | 1–2 天 | 完成 |
| 1 | workspace、空宿主、渲染門面三件套與立方體 | headless CI 通過；核心／內容無法依賴 wgpu | 約 1 週 | 待開始 |
| 2 | `module.ncl` → `lattice-compose` → resolver／lock；穩定數值 ID；CLI 雛形 | 亂序註冊不改 ID；熱路徑只見 `BlockId` 等數值型別 | 1–1.5 週 | 未開始 |
...
| 7 | 同一第二內容包以預編譯 dylib 放入 `modules/` | 靜／動實現得到相同 ID 與區塊雜湊；ABI 不符診斷可讀 | 1–2 週 | 未開始 |
...
加入 `lattice-compose`、`lattice-modules` 與 `lattice-cli`：合約驗證、Nickel 完整求值、正規化 JSON、精確 lock、解析圖、穩定數值 ID、靜態膠水 crate 產生。
...
- [套件管理與雙實現路徑](../architecture/package-management.md)
