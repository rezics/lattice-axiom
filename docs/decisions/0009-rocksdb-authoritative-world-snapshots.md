...
Lattice Axiom 的世界生成由可更換套件與組態組成。若只保存種子與玩家差異，生成器更新或套件移除後，同一座標可能得到完全不同的地形；已被探索的世界便不再穩定。另一方面，空間資料主要按區塊載入與卸載，不適合在 demo 中把每個方塊或一般世界實體做成關聯式資料列。

先自研 Region File 再遷移到嵌入式資料庫，會建立一套只使用一個階段、仍需自行處理索引、原子寫入、損壞復原與備份的格式。既然 World Store 的長期本機實現已選定 RocksDB，demo 直接使用它。

## 決策

1. RAM／ECS 是正在執行的世界；RocksDB World Store 是已物化空間世界的權威；未來的 PostgreSQL 管理需要關聯查詢與跨世界交易的社會性資料；物件儲存只負責備份與快照分發。
2. 區塊第一次生成完成後立即保存完整可讀快照。之後只要快照存在便直接載入，不因目前生成器、套件或組態變更而隱式重算。
...
- 已探索區域不會因套件更新而突然改變；同一世界可合法包含不同 generation epoch。
...
在可更換生成套件下，基線無法保證重現；移除舊生成器後甚至無法讀回世界。
...
- 第一次生成後更換生成器，舊區塊位元內容不變，新區塊使用新 generation lock。
